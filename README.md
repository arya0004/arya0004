# 👋 Hi, I'm Arya Manve
I am a passionate developer interested in building scalable, data-driven systems.  
I enjoy solving real-world problems using AI, data analytics, and software engineering.

## 💼 Experience

- **Frontend Lead – IEEE VIT Pune**
  - Built a production-grade Flutter application
  - Worked on architecture, backend integration, and deployment

- **Team – TEDxVITPune**
  - Managed large-scale events (250+ attendees)
  - Coordinated teams and operations

- **Industry Project – Forbes Marshall**
  - Built analytics dashboard for factory tracking & attendance
  - Translated business problems into data-driven solutions

## 🛠️ Projects

- 📈 Stock Market Analytics Dashboard  
- 🌍 AI Travel Planning Platform  
- 🔐 Decentralized Identity Verification System (Blockchain)  
- 🛡️ Cyber Threat Detection System (ML + MLOps)

## 🏆 Achievements

- Finalist – Odoo IIT Gandhinagar Hackathon  
- NVIDIA Certification – AI Anomaly Detection  

